...
bool SPIFFSEditor::canHandle(SAWServerRequest *request){
...
void SPIFFSEditor::handleRequest(SAWServerRequest *request){
  ...
  if(request->method() == HTTP_GET){
    if(request->hasParam("list")){
      ...
      if (request->header("If-Modified-Since").equals(buildTime)) {
        ...
        SAWServerResponse *response = request->beginResponse_P(200, "text/html", edit_htm_gz, edit_htm_gz_len);
...
void SPIFFSEditor::handleUpload(SAWServerRequest *request, const String& filename, size_t index, uint8_t *data, size_t len, bool final){
  if(!index){
